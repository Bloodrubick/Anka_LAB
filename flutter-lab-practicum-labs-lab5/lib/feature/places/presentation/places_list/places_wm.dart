...
final class PlacesWM implements IPlacesWM {
  ...
  Future<void> onFilterPressed() async {
    ...
    final newFilter = await _context.router.push(FilterRouteBuilder(initialFilter: filter));
  ...
  void onPlacePressed(PlaceEntity place) {
    unawaited(
      _context.router.push(PlaceDetailRouteBuilder(place: place)),
    );
  }
  ...
  Future<void> onSearchPressed() async {
    ...
    if (!_context.mounted) return;
    unawaited(
      _context.router.push(SearchRouteBuilder(initialFilter: filter)),
    );
  }
